...
# # view.py (The relevant sections)

# # ... (Imports and styling functions remain the same) ...
...
#         # NEW: Variable to store the ID of the scheduled 'after' call
#         self.after_id = None 

#         # ICON FIX: Directly call iconbitmap on self (the root window)
#         icon_path = os.path.join(r'C:\Users\miste\OneDrive\Desktop\TIME_CLOCK\images', 'clock_image.ico')
#         if os.path.exists(icon_path):
#              # NOTE: iconbitmap path is usually relative or needs to be absolute and correct.
#             try:
#                 self.iconbitmap(icon_path)
#             except tk.TclError:
#                 print("Warning: Could not load icon.")
            
#         # --- Register the clean shutdown handler (CRITICAL FIX) ---
...
#     def on_closing(self):
#         """Called when the user attempts to close the window, ensures clean shutdown."""
#         # Explicitly cancel the next scheduled update_clock call
...
# # ... (Individual Frame classes remain the same) ...
